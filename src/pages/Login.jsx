...
//	const history = useHistory()
	...
	const formSubmitter = e => {
		e.preventDefault();
		setsuccessMessage('');
		if (input.email=="") return seterrorMessage("Please entre your User ID");
		if (input.password=="") return seterrorMessage("Please Enter password");
		if (!emailValidator(input.email)) return seterrorMessage('Please enter valid email id');

		if (!passwordValidator(input.password))
			return seterrorMessage(
				'Password should be minimum 6 char. long with at least one number and one alphabet'
			);
					...
					<div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
						<form className="login100-form validate-form" onSubmit={formSubmitter}>
							...
							<div className="wrap-input100 validate-input" data-validate="Password is required">
						     ...
						     <div><Checkbox />
							 <label className='label-input100'>Remember me</label></div>
